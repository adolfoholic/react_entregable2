  ...
  return (
    <article className='card'>
        ...
        <section className='card__first-section'>
            <img className='card__icon'src={weather ? `http://openweathermap.org/img/wn/${weather?.weather[0].icon}@2x.png`:"No hay información"} alt="" />
        ...
        <section className='card__second-section'>
            ...
            <ul className='second__list'>
                ...
                <li className='second__item'><span className='second__span'>Presure:</span> {weather?.main.pressure} hPa</li>
            </ul>
        </section>
        <h2 className='card__temperature'>{isCelsius ? `${temperature?.celsius} °C 🌡` : `${temperature?.farenheit} °F 🌡`}</h2>
        <button className='card__btn' onClick={changeTemperature}>{isCelsius ? 'In Farenheit' : 'In Celsius'}</button>
    </article>
  )
}
